---
title: "Análisis delitos forestales cantones Osa y Golfito"
...
El SITADA clasifica como tipo de ingreso "Forestal" a 8 tipos de infracciones, en las cuales se centra este análisis.
...
data <- read.xlsx(xlsxFile = "delitos_TODOS_18abril24.xlsx", fillMergedCells = TRUE, colNames = TRUE)%>%
  mutate (convertToDate(Fecha.de.Ingreso))

data <- data[, -7] # remover una de las columnas Estado, que está duplicada
data$Fecha.de.Ingreso <- convertToDate(data$Fecha.de.Ingreso)# dar formato de fecha
data <- data%>%
  separate(Localidad, c("Localidad", "Cantón", "Provincia"), ",")

## Datos para Osa y Golfito

cantones <- c(" Golfito", " Osa")

data_OSAGOLFITO <-subset(data, Cantón %in% cantones,
                         select= c(Infracción:Estado)) #Separar datos para Golfito y Osa
...
delito_forestal <- subset(data_OSAGOLFITO, Infracción %in% forestal,select= c(Infracción:Estado))
delito_forestal$year <- substr(delito_forestal$Fecha.de.Ingreso, 1,4) #agregar columna con el año
